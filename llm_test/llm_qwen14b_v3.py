...
def wenben(query, answer, display_level):
    ...
    # 展示级别=1，代表不确定能不能回答问题，就会出现回答不了的情况，
    # 那么要给出辅助信息输出（见要求2）
        ...
        prompt = '你是专业资深严谨的金融分析师,请只根据资料给出专业深入的回答.\n\n' \
                 ...
                 '-----------------------------------\n#####根据上述信息回答问题:\n{question}\n' \
                 '\n回答要求如下：\n' \
                 '1. 只根据上述资料,注意*识别时间、实体、关键信息*维度上<信息>和<问题>的逻辑关系，然后选用正确信息，最后正确、详细、清晰、完整、高质量的回答问题。\n' \
                 '2. 如果根据上述资料无法准确回答问题，请在*开头*输出“我无法准确回答你的问题，但我可以为你提供如下帮助：”，然后再输出有帮助的信息。\n' \
                 '3. 理解用户无法看到你知道的信息,所以回答中禁止输出“根据资料”这种字眼，不要输出资料来源，不要输出资料中的“图表”等信息，直接回答问题。\n' \
                 '\n*提示*：今年是2023年，去年则代表着2022年；注意完整的回答问题，甄别实体和关键信息是否匹配。\n' \
                 '下面请给出你对问题的回答:  '.format(question=query, answer=answer)
        print(prompt)
        res = qwen14b(prompt, 0.7, 0.95)
        res = res.replace('根据上述信息，', '').replace('根据资料，', '').replace('根据信息，', '').replace('从上述信息来看，',
                                                                                                     '').replace(
        '从上述资料来看，', '').replace('从资料中可以看出，', '').replace('根据提供的信息，', '')
    return res


if __name__ == '__main__':
    # 加载检索数据
    data = load_json_data('out_for_llm_test.jsonl')
    # print(data)
    ...
    for item in data:
        ...
        评估规则 = (
            ...
            4. prompt还有没有优化空间
            """)
        gpt4_out = parse.parse_func(
            f"请根据问题<{item['query']}>和检索数据<{item['hit_answer']}>评估一下模型的输出<{out}>效果,按照如下规则:{评估规则}")

        print("gpt4_out:", gpt4_out)
